...
def test_compare_separate_and_together():
    ...
    CircuitRunner(state_processor_gens=[NaiveMeasurement()]) | node0
    expv0 = node0()
    mean0 = expv0.value()
    ...
    node1.shot_num.set_value(shot_allocate)
    CircuitRunner(state_processor_gens=[NaiveMeasurement()]) | node1
    expv1 = node1()
    expv1_sum = Value.unary_operator(expv1, jnp.sum)
    mean1 = expv1_sum.value()
    var1 = expv1_sum.var.value()
    assert_allclose(mean1, mean0)
    assert var0 < var1  # To ensure NaiveMeasurement always underestimate variance
    assert_allclose(var1, var0, rtol=0.005)
